 ...
 * use { HorizontalScrollView} instead.</p>
 *
 * <p>Never add a {@link androidx.appcompat.widget.RecyclerView} or { ListView} to
 ...
 */
public class MyScrollView2 extends MyFrameLayout {
    ...
    /**
     * 如果用户正在拖动ScrollView，则为true。这个和滚动不同，滚动可以被mScroller.isFinished判断
     * （当用户的手指离开的时候才会触发滚动）
     */
    ...
    /**
     * 设置ScrollView的唯一一个子view是否填充满ScrollView
     * <p>
     * 为什么添加了这么个方法呢，我可以直接在ScrollView的布局里面直接指定子view是否充满
     */
...
//        this(context, attrs, com.android.internal.R.attr.scrollViewStyle);
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        Log.e("FFF", "MyScroll----->" + getStringMode(widthMeasureSpec));
        Log.e("FFF", "MyScroll----->" + getStringMode(heightMeasureSpec));
        Log.e("FFF", "       mFillViewport       ------>" + mFillViewport);
        ...
        if (getChildCount() > 0) {
            ...
            final MyFrameLayout.LayoutParams lp = (LayoutParams) child.getLayoutParams();
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
         ...
         * scrolling there.
         *
         * 这个方法仅仅表明我们是否阻断这个运动事件。如果我们返回true,onMotionEvent将会被调用，
         * 实际上我们是在这里做的滑动
         *
         */
        ...
        if ((action == MotionEvent.ACTION_MOVE) && (mIsBeingDragged)) {
            Log.e("FFF", "onInterceptTouchEvent---->" + "正在滑动，并且在拖拽   拦截");
            return true;
        }

        if (super.onInterceptTouchEvent(ev)) {
            Log.e("FFF", "onInterceptTouchEvent---->" + "父类拦截了事件");
            return true;
        }
        ...
        switch (action & MotionEvent.ACTION_MASK) {
            ...
            case MotionEvent.ACTION_DOWN: {
                final int downX = (int) ev.getX();
                final int downY = (int) ev.getY();

                //如果按下的这个点不在这个唯一的子view上
                if (!inChild(downX, downY)) {
                    //用户是否正在拖拽
                ...
                /*
                 * 如果正在滚动并且用户触摸了屏幕，重新初始化drag.否则不重新初始化。
                 * 当正在滚动的时候，mScroller.isFinished应该为false。需要首先调用computeScrollOffset
                 * 那样isFinished才会正确
                 *
                 */
                //当你想知道最新的位置的时候调用这个方法，当它返回为true的时候，说明滚动过程还未完成
    ...
    public boolean onTouchEvent(MotionEvent ev) {
        ...
        switch (actionMasked) {
            case MotionEvent.ACTION_DOWN: {
                ...
                /*
                 * 如果正在滚动，这时候用户突然按下，则停止滚动。如果正在滚动，isFinished方法返回结果是false
                 */
            ...
            case MotionEvent.ACTION_MOVE:
                ...
                if (mIsBeingDragged) {
                    ...
                    if (overScrollBy(0, deltaY, 0, getScrollY(), 0, range, 0, mOverscrollDistance, true) && !hasNestedScrollingParent()) {
...
//    }
    ...
    public CharSequence getAccessibilityClassName() {
        return MyScrollView2.class.getName();
    }
...
//    }
    ...
    protected void measureChild(View child, int parentWidthMeasureSpec, int parentHeightMeasureSpec) {
        ...
        childHeightMeasureSpec = MeasureSpec.makeMeasureSpec(Math.max(0, MeasureSpec.getSize(parentHeightMeasureSpec) - verticalPadding), MeasureSpec.UNSPECIFIED);
        child.measure(childWidthMeasureSpec, childHeightMeasureSpec);
    }
    ...
    protected void measureChildWithMargins(View child, int parentWidthMeasureSpec, int widthUsed, int parentHeightMeasureSpec, int heightUsed) {
        ...
        final int usedTotal = getPaddingTop() + getPaddingBottom() + lp.topMargin + lp.bottomMargin + heightUsed;
        final int childHeightMeasureSpec = MeasureSpec.makeMeasureSpec(Math.max(0, MeasureSpec.getSize(parentHeightMeasureSpec) - usedTotal), MeasureSpec.UNSPECIFIED);
